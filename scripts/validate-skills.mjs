#!/usr/bin/env node
// validate-skills.mjs — structural lint for every skills/*/SKILL.md.
// Enforces the SKILL.md frontmatter contract (name / description) and a body-length
// budget so authoring mistakes fail fast in CI. Run from the repo root:
...
const NAME_RE = /^[a-z0-9-]{1,64}$/;          // Anthropic: lowercase letters, numbers, hyphens; <=64
const RESERVED = ["anthropic", "claude"];      // Anthropic: reserved words disallowed in name
const MAX_DESC = 1024;                          // Anthropic: description max length
const MAX_BODY_LINES = 500;                     // Anthropic: keep SKILL.md body under ~500 lines
...
function parseFrontmatter(text) {
  ...
  for (const raw of lines.slice(1, end)) {
    ...
    else if (currentKey && raw.trim()) { fm[currentKey] += " " + raw.trim(); }  // folded value
  }
  return { fm, bodyLines: lines.length - (end + 1) };
}

if (!existsSync(SKILLS_DIR)) {
  console.error(`No ${SKILLS_DIR}/ directory found. Run from the repo root.`);
  process.exit(1);
}
...
for (const dir of dirs.sort()) {
  ...
  checked++;
  const { fm, bodyLines } = parseFrontmatter(readFileSync(path, "utf8"));
  const errs = [];
